...
import com.sdk_jigar_demo.jigar_sdk_sample.data.Error
import com.sdk_jigar_demo.jigar_sdk_sample.network.customs.DownloadCertificateTask
import com.sdk_jigar_demo.jigar_sdk_sample.network.customs.EncryptRequestData
import com.sdk_jigar_demo.jigar_sdk_sample.network.customs.Jwt
import com.sdk_jigar_demo.jigar_sdk_sample.network.customs.PeerCertificateExtractor
...
import com.sdk_jigar_demo.jigar_sdk_sample.utils.CustomFunctions.keyThree
					...
					if (MainAPIResponse.code == 500) {
						if (MainAPIResponse.error == null) {
							MainAPIResponse.error =
								Error(userMessage = MainAPIResponse.message)
	...
	private fun handleException(
		...
		return when (e) {
			...
			is SocketTimeoutException, is UnknownHostException -> {
				return createEmptyResponse(chain,
                    Constants.NO_INTERNET_MESSAGE,errorType = Constants.UNKNOWN_HOST_EXCEPTION)
			}
			is SSLException -> {
				cert_count++
				prefManager.setCertSha(null)
